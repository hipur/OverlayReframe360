...
# CMake configure/build/install commands land here once CMakeLists.txt exists — see specs/01-windows-port.md
```
...
Build with `-DDEBUG` (`make DEBUG=1`) to get spdlog sinks: `Reframe360Factory.cpp:init_log()` writes to
...
Three sets of headers are generated by the build *and* committed to git. If you edit a source of one,
regenerate — do not hand-edit the `.h`:
